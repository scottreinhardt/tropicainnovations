...
class ProxyScraper:
        ...
        try:
            ...
            for row in reader:
                if row:  # Ensure the row is not empty
                    ...
                    self.superDict[icao_corrected] = [float(lat), float(lon), tempDewRaw ,icao_corrected, station_name, cloudCoverWxType, weather_phenomena, time_of_day, urls, metar, is_daytime]
                    k += 1
    ...
    def is_daytime(self, lat, lon, dt_utc):
        try:
            ...
            sunrise = s['sunrise']
            sunset = s['sunset']
            sunrise_str = sunrise_dt.strftime("%d%H%MZ")
            sunset_str = sunset_dt.strftime("%d%H%MZ")
            print
            #datetime_match = self.get_time(str(metar)).group()
            sunrise_utc_time = self.to_metar_zulu_format(sunrise_str)
            sunset_utc_time = self.to_metar_zulu_format(sunset)

            sunrise_local_time = self.get_local_hour_from_metar_zulu(sunrise_str, lon)
            sunset_local_time = self.get_local_hour_from_metar_zulu(sunset_utc_time, lon)
            print(f"utc sunset time: {sunrise_str}")
            print(f"local sunset time: {sunset_local_time}")

            print(f"sunrise utc time: {sunrise_utc_time}")
            print(f"sunrise local time: {sunrise_local_time}")
            #local_sunset =
            #get_local_hour_from_metar_zulu(
            print(f"Sunrise: {sunrise}, Sunset: {sunset}, UTC Time: {dt_utc}")
            return sunrise <= dt_utc <= sunset
        except Exception as e:
    ...
    def is_daytime(self, lat, lon, dt_utc):
        try:
            ...
            # Get today's and yesterday's sun data
            ...
            # Handle early-morning UTC case: use yesterday's sunset
            if dt_utc < sunrise:
                sunset = s_yesterday['sunset']
                sunset_formatted = self.to_metar_zulu_format(sunset)
                sunset_local_time = self.get_local_hour_from_metar_zulu(sunset_formatted, lon)
            # Handle cases when the sunset is after midnight the next day in UTC
            elif dt_utc > s_today['sunset']:
                # Change the day of the month in the datetime object to one day ahead
    ...
    def is_daytime(self, lat, lon, dt_utc):
        try:
            observer = Observer(latitude=lat, longitude=lon)
            s = sun(observer, date=dt_utc.date())  # Use default timezone-aware sunrise/sunset

            # Get local offset in hours (rough approximation)
            ...
            print(f"Sunrise (local): {sunrise}, Sunset (local): {sunset}")
    ...
    def generate_map(self):
        ...
        for icao, data in self.superDict.items():
            try:
                lat, lon, temp_c, icao_corrected, station_name, cloudCoverWxType, weather_phenomena, time, urls, metar, is_daytime = data
                #print(cloudCoverWxType)
                #print(metar)
                ...
                folium.Marker(
                    ...
                    popup=folium.Popup(html=f"""
                        ...
                        <div style="width: 100%; height: 15px; background: {gradient_css_temperture}; border-radius: 5px; position: relative;">
                            <div style="position: absolute; left: {((temperature + 40) / 155) * 100}%; top: -5px; width: 3px; height: 25px; background: black;"></div>
                        </div>

                        <!--<input type="range" min="-40" max="120" value="{temperature}" disabled style="
                            width: 100%;
                            height: 15px;
                            /*
                            ...
                            background: {gradient_css_temperture};
                        ...
                        <input type="range" min="-40" max="120" value="{dewpoint}" disabled style="
                            width: 100%;
                            height: 15px;
                            background: linear-gradient(to right,
                              #000000 0%,        /* -40 */
                             ...
                             /*background: {gradient_css_dewpoint};*/
