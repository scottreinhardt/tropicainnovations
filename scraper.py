...
class ProxyScraper:
    ...
    def cloudCoverType(self, row):
        ...
        if finalCloud == []:
            finalCloud.append("No Cloud Cover Reported")
    ...
    # cloud_cover_type is the aray of [cloud cover, cloud height * 1000] which is determined in the parse_data method
    ...
    def average_cloud_condition(self, cloudCoverType):
        ...
        # INdicates more than one cloud level reported in the METAR
            ...
            if ccArr:
                return (sum(ccArr) / len(ccArr)) * 100
            else:
    ...
    def cloud_cover_percentage_to_text(self, numerical_avg_cloud_cover):
        if numerical_avg_cloud_cover <= 0.125:
            # CLR conditions
            ...
            for row in reader:
                if row:  # Ensure the row is not empty
                    ...
                    if len(row) > 6:
                        ...
                        raise IndexError("Row does not contain enough data.")
                    """
                    """
                    try:
                        lat = float(row[3])
                        ...
                        print(f"Skipping airport due to missing or invalid lat/lon/temp/dew data.")
                        continue
                    """
                    #windkts = 0.0
                    #windgustkts = 0.0
    ...
    def generate_map(self):
        ...
        for icao, data in self.superDict.items():
            try:
                ...
                """
                if wx_phenomena == "" or None:
                    wx_phenomena = "None"
                else:
                    wx_phenomena = wx_phenomena[0]
                """
        ...
        m.save(file_path)
        #m.save("/static/METAR_map/airport_metar_map.html")
        #m1.save("airport_metar_map1.html")
"""
def national_currentweather_map(self):
    # Import Basemap from mpl_toolkits
    from mpl_toolkits.basemap import Basemap
    import matplotlib.pyplot as plt

    # Correct the Basemap initialization using the EPSG code (4326 in this case)
    m = Basemap(projection='mill', llcrnrlon=-123., llcrnrlat=37, urcrnrlon=-121, urcrnrlat=39, resolution='l', epsg=4326)

    # Optionally, you can still use a background map
    m.arcgisimage(service='World_Physical_Map', xpixels=5000, verbose=False)

    # Plot each station from the superDict
    for icao, data in self.superDict.items():
        lat, lon, temp_c, icao_corrected, station_name, cloudCoverWxType = data
        temperature = temp_c[0]
        dewpoint = temp_c[1]
        ...
        # Plot the point
        m.plot(x, y, 'ro', markersize=3)  # Red points for stations
