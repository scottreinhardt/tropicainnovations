...
class ProxyScraper:
    ...
    def create_svg_dial(self, value, categories):
        ...
        svg_parts = [f'<svg width="250" height="300" viewBox="0 0 250 200">']
        ...
        for i, (label, color) in enumerate(categories):
            ...
            svg_parts.append(f'<path d="{path_d}" stroke="{color}" stroke-width="20" fill="none" stroke-linecap="round" />')
        ...
        index = int((value / 100) * len(categories))
        ...
        svg_parts.append(f'<text x="{cx}" y="125" text-anchor="middle" class="gauge-text" font-size="20" font-family="Arial">{value}%</text>')
        svg_parts.append(f'<text x="{cx}" y="150" text-anchor="middle" class="gauge-label" font-size="16" fill="#333" font-family="Arial">{index}</text>')
    ...
    def generate_legend(self):
        categories = [
            {"label": "Frigid", "range": "-40° to 20°", "color": "#1a1a1a"},
            {"label": "Cold", "range": "20° to 50°", "color": "#4ab7ff"},
            {"label": "Comfortable", "range": "50° to 75°", "color": "#56d18f"},
            {"label": "Warm", "range": "75° to 90°", "color": "#ffb347"},
            {"label": "Hot", "range": "105° to 120°", "color": "#f44336"},
        ]

        svg_parts = []
        x_start = 50  # starting x
        y_base = 250  # base y for the boxes
        # Measured in pixels
        rectangle_length = 100
        # Measured in pixels
        space_btwn_categories = 30
        for i,cat in enumerate(categories):
            # Define the start of each rectangle
            # x_start is the position of the beginning of the rectangle on ththe x axis
            # i is index of current category in lost of categories
            x = x_start + i * (rectangle_length + space_btwn_categories)
            box = f'''
            <rect x="{x}" y="{y_base}" width="100" height="20" rx="10" ry="10" fill="{cat['color']}"/>
            <text x="{x + 50}" y="{y_base + 35}" font-size="12" text-anchor="middle" fill="black">{cat['label']}</text>
            <text x="{x + 50}" y="{y_base + 50}" font-size="12" text-anchor="middle" fill="black">{cat['range']}</text>
            '''
    ...
    def generate_legend(self):
        ...
        for i, cat in enumerate(categories):
            x = x_start + i * (rectangle_length + space_btwn_categories)
            box = f'''
            <rect x="{x}" y="{y_base}" width="40" height="15" rx="4" ry="4" fill="{cat['color']}"/>
            <text x="{x + 20}" y="{y_base + 25}" font-size="8" text-anchor="middle" fill="black">{cat['label']}</text>
            <text x="{x + 20}" y="{y_base + 35}" font-size="7" text-anchor="middle" fill="black">{cat['range']}</text>
             '''
            svg_parts.append(box)

        final_svg = f'''
        <svg width="450" height="50" xmlns="http://www.w3.org/2000/svg">
          {''.join(svg_parts)}
        </svg>
        '''
        return final_svg
    def generate_map(self):
        ...
        for icao, data in self.superDict.items():
            try:
                ...
                svg_dial = self.create_svg_dial(temperature, categories)
        ...
        <div style="transform: translate(-50%, -50%);
                    position: relative;
                    width: 60px;
                    height: 60px;">{self.generate_thermometer_svg(temperature, dewpoint)}</div>
    """,
                    icon_size=(30, 30),  # width, height of the div
                    icon_anchor=(15, 15)  # center of the div (half width and height)
                    ),
