...
class HRRR_Temperature_Map:
    ...
    AE	28119 95762	Al Ras		03		100	Al Ras	112	25.2684	55.2931	6
    AE	28120 95556	Al Ras		03		100	Al Ras	112	25.2666	55.2932	6
    AE	28145 95641	Al Ras		03		100	Al Ras	112	25.2673	55.2934	6
    AE	28150 95596	Al Ras		03		100	Al Ras	112	25.2669	55.2934	6
    AE	28174 95593	Al Ras		03		100	Al Ras	112	25.2669	55.2937	6
    AE	28178 95711	Al Ras		03		100	Al Ras	112	25.268	55.2937	6
    """
    ...
    def get_forecast_fast(self):
        ...
        airport_indices = {}
        for i, (icao, data) in enumerate(self.airportLocations.items()):
            lat = data[0]
            lon = self.normalize_lon(data[1])
            _, idx = tree.query([lat, lon])
            y_idx, x_idx = np.unravel_index(idx, lats.shape)
            ...
            print(f"[Indexing] {i + 1}/{len(self.airportLocations)} ({((i + 1) / len(self.airportLocations)) * 100:.2f}%)")
        ...
        total_steps = len(hours) * len(self.airportLocations)
        step = 0
        ...
        for hour, ds in enumerate(model_arr):
            ...
            for icao, (y_idx, x_idx) in airport_indices.items():
                ...
                dew_f = round((dew_k - 273.15) * 9/5 + 32)
                self.airportLocations[icao][7][hour] = dew_f
                step += 1
    ...
    def json_dump(self):
        with open('/home/tropicainnovations/mysite/static/hrrr_grib_json_dump/hrrr_dump.json', "w") as f:
            json.dump(self.airportLocations, f)
