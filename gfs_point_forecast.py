...
class GFS_Point_Forecast:
    ...
    def fetch_airport_data(self):
        # /home/tropicainnovations/mysite/static/METAR_map/stations_list/stations.cache.json
        ...
        try:
            with open(file_path, 'r') as file:
                ...
                for city in city_data:
                    ...
                    """
                    if timezone:
                        # currently timezones are written like "America New York" and need to be written as "America/New_York"
                        # remove any leading or training whitespace from the timezone " America New York " to "America New York"
                        # Replace any spaces with underscores. "America New York" becomes "America_New_York"
                        # Split timezone at the first space only
                        timezone = timezone.strip().replace(" ", "_")
                        # Split at the first underscore
                        # Example: "America_New_York" becomes ["America", "New York"]
                        parts = timezone.split("_", 1)
                        # Double check that parts has a length of 2
                        if len(parts) == 2:
                            # Join parts back together but with a /
                            # ["America", "New_York"] becomes "America/New_York"
                            timezone = parts[0] + '/' + parts[1]
                    """
                    ...
                    #p_rate = {}
                    #p_type = {}
                    self.cityLocations[city_name] = [lat, lon, country_name, timezone, [], [], [], [], []]
    ...
    def get_forecast_fast(self):
        ...
        latest_date, latest_run = self.get_latest_gfs_run()
        ...
        total_hours = np.arange(starting_fxx, starting_fxx + 24, new_time_step)
        ...
        for ds, ds_dew, ds_u_wind, ds_v_wind, ds_cc, hour in zip(model_arr, model_arr_dew, model_arr_wind_u, model_arr_wind_v, model_arr_cc,  valid_hours):
            ...
            for icao, (y_idx, x_idx) in city_indices.items():
                ...
                city_data[icao][4].append(temp_f)


                # Handle dewpoints
                dew_k = dew_data[y_idx, x_idx]
                dew_f = round((dew_k - 273.15) * 9/5 + 32, 2)
                city_data[icao][5].append(dew_f)
...
if __name__ == '__main__':
    GFS_Point_Forecast = GFS_Point_Forecast()
    ...
    GFS_Point_Forecast.read_json('/home/tropicainnovations/mysite/static/geonames-all-cities-with-a-population-1000.json')
    GFS_Point_Forecast.get_forecast_fast()
    GFS_Point_Forecast.json_dump()
